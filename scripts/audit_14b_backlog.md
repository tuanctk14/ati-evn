...
- [RESOLVED] **Manual test 7.4**: chaining 2 destructive actions in 1 analyst turn -- policy change + a real code-layer bug fixed
  Discovered running Nhom test 7 (Human-in-the-loop) scenario "Dong Finding X va danh dau Finding Y la false positive" against a live Bot 2. First attempt tried to BAN chaining entirely (matching the original `SYSTEM_PROMPT` rule "NEVER chain more than one destructive tool call in the same turn"), adding a code-layer guard (`destructive_pending_in_turn` in `function_calling.py`/`react.py`) plus a duplicate-propose replay path in `_action_base.py`. That combination surfaced a new failure mode (`TOKEN_SOFT_CAP` hit after only 3 tool calls, aborting the flow) and was reverted back to baseline (commit `54a4d01`) rather than ship half-verified.
  Policy then changed (analyst decision): chaining IS allowed -- proposing multiple PENDING_CONFIRMATIONs in one turn is safe (nothing executes yet); only executing more than one without individual confirmation would not be. `SYSTEM_PROMPT` (config.py) rewritten accordingly: the model may call each named destructive tool without `confirmed=True` in the same turn, present all pending actions together itemized, and on a single "xac nhan" re-call ALL of them with `confirmed=True` (not just the first).
  Testing this new policy surfaced the REAL bug: `_action_base.py`'s `_find_pending_for_tool()` fallback (used when the model's confirmed=True re-call has a `reason` string that doesn't hash-match the original pending args verbatim, e.g. "Analyst requested closure" vs the original "Yeu cau dong finding...") returned ALL pending entries for a given `(session, tool_name)` with no way to disambiguate -- so with 2 different Findings (#255, #257) simultaneously pending for the same tool `update_finding_status`, the analyst's single "xac nhan" hit `len(candidates) > 1` and failed with "Multiple pending confirmations exist... ambiguous", even though the model's confirm call still correctly included the right `finding_id` for each. Fixed by having `_find_pending_for_tool()` accept the model's current confirm-call kwargs and narrow candidates by any overlapping identifying keys (excluding `reason`/`confirmed`) before declaring ambiguity -- only returns multiple candidates (genuine ambiguity) when no single candidate is a strictly-better match than the rest. Verified end-to-end via live Bot 2 with a fresh `/reset`: "Dong Finding 255 va danh dau Finding 257 la false positive" -> both proposed together, itemized, one "xac nhan" -> both executed correctly (DB confirmed: #255 CLOSED, #257 FALSE_POSITIVE), no ambiguous-match error.


- [FIXED] **Nhom test 8.1**: how-to question ("Lam sao de dong Finding nay?") misclassified as a real destructive request
  Testing the postfilter scenario "Lam sao de dong Finding nay?" (a general how-to/help question, using vague "nay" with no specific Finding named) against a live Bot 2: instead of explaining the close workflow in the abstract, the agent resolved "Finding nay" via session context's `last_finding_id` (from an earlier, unrelated turn) and immediately called `update_finding_status(finding_id=261, new_status=CLOSED)` in PENDING_CONFIRMATION mode -- i.e. it treated a HOW-TO QUESTION as an actual close request pending only the analyst's confirmation. No execution happened that time only because the analyst noticed before confirming and separately because the pending entry's 5-minute TTL expired first -- the underlying misclassification was unfixed and would have executed the close if "xac nhan" had landed within 5 minutes. Root cause: `agent/loop/config.py`'s "If the analyst's request is an action a tool can perform... you MUST call that tool IMMEDIATELY" rule (CRITICAL RULES section) had no distinction between a REQUEST to act and a QUESTION about how acting works, so it fired on any message mentioning an action verb.
  Fixed: added an explicit rule ahead of the existing one in `SYSTEM_PROMPT` telling the model to first classify REQUEST ("Dong Finding 261", imperative + named target) vs QUESTION ("Lam sao de dong Finding?", "huong dan...", "phai lam gi", no specific target) -- only call the action tool for the former; for the latter, explain the workflow in the abstract and ask for a specific target instead of resolving one from stale session context. Also states explicitly why this matters (creates a live PENDING_CONFIRMATION the analyst could accidentally confirm, not just an unhelpful answer).
  Verified via direct `run_agent()` calls (not live bot, to respect the in-progress 7.5/10.4 TTL test's no-restart constraint), two cases: (1) same how-to question with `session.last_finding_id=261` pre-set (reproducing the exact failure condition) -> agent made ZERO tool calls, correctly explained "Đây là câu hỏi hướng dẫn (chưa có Finding cụ thể nào được nêu)" and asked for a specific id; (2) a real request with a named target ("Dong Finding so 253") -> agent correctly called `get_finding_detail` then `update_finding_status(confirmed=False)`, proposing PENDING_CONFIRMATION as designed -- confirms the fix distinguishes the two cases rather than over-blocking real requests.

- [FIXED] **Nhom test 8.5**: slash-commands with underscores rendered WITHOUT underscores on live Telegram -- root cause was Telegram's Markdown parser, not postfilter.py
  Initially misdiagnosed: hallucinated slash-commands without underscores (`/addcustomer`, `/markfp`, `/confirmcampaign`, etc.) appeared verbatim in a live Bot 2 answer to "Cac lenh admin gom nhung gi?", alongside a trace line `[postfilter fixed: ]` with an empty fix list -- isolated retesting of `postfilter_answer()` with a manually-retyped approximation of the observed text showed it correctly stripping these (contradicting the live behavior), and the root cause could not be identified in that first pass.
  Root-caused properly by adding a temporary debug log of the exact `answer` text immediately BEFORE `postfilter_answer()` runs, then reproducing live via Bot 2: the log revealed the agent's answer text actually contained CORRECT commands with underscores intact (`/list_open`, `/mark_fp`, `/add_customer`, ...) -- `postfilter_answer()` was never the bug, it correctly left these alone (they matched `WHITELIST`). The underscores were being silently swallowed by Telegram's own **legacy Markdown renderer** at `parse_mode="Markdown"` (`agent_handler.py`'s `_send_markdown()`) -- Telegram's legacy Markdown reads a bare `_` as an italic-span delimiter, so `/list_open` renders as `/listopen` with the underscore consumed as markup rather than displayed. `postfilter.py` already had a similar protection for prose snake_case (`_SNAKE_CASE_RE`, e.g. `risk_score` -> "risk score") but explicitly EXCLUDED anything preceded by `/`, specifically so `postfilter_answer()`'s own `WHITELIST`/`FIX_MAP` matching (case-sensitive-preserving, exact-string) wouldn't break -- that exclusion was correct for its original purpose but left slash-commands exposed to the Markdown-swallowing bug.
  Fixed (agent/loop/postfilter.py): after `postfilter_answer()`'s WHITELIST/FIX_MAP matching completes (so matching still sees the exact unescaped token), a final pass escapes `_` to `\_` in every surviving `/command` token, which Telegram's Markdown parser renders as a literal underscore instead of italic markup. Verified: matching logic unaffected (`/list_open` etc. still correctly recognized as whitelisted, not stripped; `/campaign_detail` etc. still correctly replaced via FIX_MAP) and output now contains `/list\_open`, `/add\_customer`, etc. Verified live via Bot 2 (fresh `/reset`, same failing question, 3rd repro after the fix): full answer now displays with underscores intact for every real command (`/list_open`, `/mark_fp`, `/add_customer`, `/confirm_campaign`, `/search_indicators`, ...), and the trace line correctly shows all 4 hallucinated commands from that run being replaced (`/report->/generate_report`, `/campaign_detail->/campaign`, `/campaign_confirm->/confirm_campaign`, `/show_campaign->/campaign`). Also fixed as a byproduct: `agent_handler.py`'s trace line now surfaces `filter_stats["stripped"]` (as `bad->(removed)`) alongside `replaced`, so a stripped-only turn no longer shows a misleadingly-empty `[postfilter fixed: ]`.

- [FIXED] **Nhom test 5 retest**: search_findings hallucinated `offset` param + no pagination support -- aggregate/group-by questions structurally can't get complete data
  Re-running the full Nhom test 5 (15 cau) via `test_agent.py` to get fresh timing/tool-call data post-fixes surfaced: "Tong hop Finding theo don vi trong thang nay" (Nhom 5.1 cau 5) failed function-calling entirely (2 consecutive timeouts, one preceded by a "backend-incompatibility 400" DFLASH-routing retry), then ReAct fallback also errored mid-run -- `search_findings(since_days=30, limit=200, offset=20)` -> `TypeError: search_findings() got an unexpected keyword argument 'offset'` (model tried to paginate past the tool's `HARD_CAP=20` per call, a reasonable strategy for a 199-Finding dataset, but the tool has no `offset` param to support it). ReAct recovered from the error (continued past it) but the final answer was visibly TRUNCATED mid-sentence ("Tong cong" cut off) -- likely `max_tokens`/response-length limit hit while emitting a 13-row markdown table, not investigated further this session. This explains why the SAME question via function-calling earlier in this session took a different, more expensive route: 8 separate `search_findings(customer=X, limit=1)` calls (one per customer) to get a `total_count` per group instead of paginating a single list -- a workaround for the same missing-pagination gap, costing 80,257 tokens.
  Fixed: added real `offset` param to `search_findings` (agent/tools/search_findings.py) -- `HARD_CAP=20` per call unchanged (still can't be raised via `limit`), but `offset` now lets repeated calls page through the full result set (offset=0, 20, 40, ...) without hitting the prior `TypeError`. Updated the tool's description to explicitly document this pagination pattern for the model, and added a `SYSTEM_PROMPT` tool-selection rule (agent/loop/config.py) pointing group-by/breakdown questions at paginated `search_findings` instead of the per-customer `limit=1` workaround. Verified directly (not via live bot, to avoid disrupting the in-progress 7.5/10.4 TTL test): `search_findings(status="open", limit=20, offset=0)` and `offset=20` return disjoint id sets (verified via set intersection = empty), `total_count` stays consistent across pages. The ReAct truncated-answer symptom (b) is NOT fixed by this -- left open, separate root cause, not yet investigated.

- [MOSTLY FIXED] **Aggregate-query token budget: consolidated finding, resolved for the worst case via a strategy fix rather than a token-budget fix**
  Root cause: `SYSTEM_PROMPT` (~6,240 tokens after an in-session trim) + all 58 tool JSON schemas (~9,956 tokens, untouched) = ~16,200 tokens re-sent UNCHANGED on every LLM call within a function-calling turn. A 3-step turn (2 tool calls + answer) costs >=48,600 tokens on overhead alone -- already brushing `TOKEN_SOFT_CAP=50_000` with zero real data involved. Symptoms observed repeatedly this session: 5+ of ~13 distinct ordinary questions hitting the token cap or a full timeout; the SYSTEM_PROMPT trim alone giving inconsistent real-world improvement (step-count variance dominates prompt-size savings); "Tong hop Finding theo don vi" (12-customer aggregate) failing outright (10-call pagination, 118,211 tokens) or truncating mid-answer even after the `search_findings`/`search_asset` offset fixes.
  Two final fixes landed and verified live via Bot 2: (1) `generate_report(scope="all")` already computes a per-customer Finding breakdown server-side (`data_gatherer.py`'s `findings_by_customer`) -- redirected the "tong hop theo don vi" tool-selection rule to call this ONE tool instead of having the model reconstruct the breakdown itself via per-customer loops or pagination; (2) added a truncation-retry to `function_calling.py`'s final-answer path (same failure-mode class as the earlier ReAct fix and `chat_json()`'s existing one) -- when `finish_reason == "length"` with non-empty content, retry once with double `max_tokens` and a longer timeout instead of silently returning a mid-sentence-cut answer.
  Verified live via Bot 2 (fresh `/reset`, same question that previously failed/truncated): now completes in **1 tool call, 37,979 tokens** (vs. 80,257 tokens/8 calls or 118,211 tokens/10 calls in earlier attempts) with a complete, accurate, non-truncated answer (breakdown by customer, severity, ATT&CK, alert-dispatch stats). Two earlier live attempts failed with a full timeout, but reproducing the identical question via CLI in parallel showed the same "DFLASH grammar-constrained decoding" backend-routing error present at that time (not related to this session's fixes) -- a third live attempt after the backend recovered succeeded cleanly, confirming the timeouts were transient backend instability, not a regression.
  What remains open (smaller, deferred): (a) the ~9,956-token tool-schema block is still the largest untouched token-budget lever -- the `generate_report` fix sidesteps it for this ONE question shape by using a purpose-built cheap tool, it doesn't reduce the schema cost for other multi-step aggregate questions; (b) `generate_report` only surfaces the top 5 customers, not all ~12-13 -- acceptable for the tested question ("dong vi nao can uu tien") but would need a different approach if an analyst explicitly wants the full unabridged list. Recommend for the thesis: this demonstrates that for at least one concrete hard case, a targeted tool-selection fix (redirecting to an existing purpose-built tool) outperformed chasing the token budget directly -- worth noting as a general lesson alongside the remaining, still-open tool-schema-size limitation.

- [FIXED] **Nhom test 8 (C8_react_comparison)**: ReAct multi-source aggregation failures -- root-caused, truncation retry + brevity guidance added
  Root-caused the ReAct failures observed in Nhom 8's comparison ("Liet ke Finding lien quan Fortinet" failing 2/2 attempts): reproduced directly via `run_react()` with INFO logging (not via live bot, to respect the in-progress 7.5/10.4 TTL test's no-restart constraint) and found the model's completion repeatedly landing EXACTLY at `max_tokens=2048` per ReAct step for this question -- two distinct symptoms of the same cause: (a) a `Final Answer:` header got emitted followed by a multi-row markdown table that then got cut off mid-sentence (FINAL_RE still matched since it only requires the header to appear, so the truncated text silently became the analyst's answer), and (b) a Thought/Action block cut off before EITHER header was written at all (matched neither FINAL_RE nor ACTION_RE, falling into "no action found -> treat as malformed" -> the generic "⚠️ Agent không tạo được câu trả lời hợp lệ" error).
  Fixed (agent/loop/react.py), two changes: (1) added a truncation-retry, same failure-mode class as `chat_json()`'s existing one -- when a step's `completion_tokens` hits `step_max_tokens` (2048) AND the response doesn't contain a complete Final-Answer-or-Action block, retry ONCE with `max_tokens=4096` and a longer `timeout=60.0` (the first attempt used the default 30s timeout, which then itself timed out on the larger completion -- 28.5s observed just under the limit -- so timeout had to be raised alongside max_tokens); (2) added explicit brevity guidance to `REACT_SYSTEM` ("Cap your Final Answer to ~400 Vietnamese words... summarize with bullet points instead of a wide markdown table... prioritize the most important 5-10 items plus a total count"), mirroring the cap `SYSTEM_PROMPT` (function-calling) already had but `REACT_SYSTEM` was missing -- markdown tables were the main driver of over-length completions.
  Verified via direct `run_react()` calls (5 repro runs across both previously-failing questions): retry-alone (before brevity guidance) improved from complete failure to a longer-but-still-occasionally-truncated answer (1615 chars, cut off mid-word); with brevity guidance added, BOTH previously-failing questions ("Liet ke Finding lien quan Fortinet" and "Tong hop Finding theo don vi trong thang nay") now complete fully without truncation (1294 and 1719 chars respectively, both ending in a complete sentence) and use bullet points instead of tables as instructed.

- [OBSERVED, not a bug] **Nhom test 9**: playbook cache HIT via agent tool did not show a speed improvement over cache MISS
  Testing "Tao playbook ung pho cho cve-2026-47295 o phan doan IT" (cau 3, cache MISS -- 38.6s) then repeating verbatim (cau 5, cache HIT confirmed via `playbook_cache.reused_count` incrementing from 0->1) took LONGER (59.2s) than the miss, contrary to the expected "cache hit = near-instant" assumption from the slash-command's own design (`/playbook`'s cache lookup is a fast DB read with no LLM call). Root cause: unlike `/playbook` (which returns the cached markdown directly to Telegram with no further LLM involvement), the `generate_playbook` AGENT TOOL's result still gets wrapped in a full function-calling turn -- the LLM still has to read the (large, ~1800-word) cached markdown as a tool result and compose its own narrative answer around it (this turn used 47,194 tokens, the highest of the 5 Nhom 9 questions), so the "cache" only saves the SIGMA/PLAYBOOK GENERATION step, not the agent's own answer-composition step. Not a bug -- both cache hit and miss go through the same agent-loop overhead -- but worth noting in the thesis as a caveat: cache-hit speedup is real at the tool/DB layer (verified via reused_count) but may not be perceptible to the analyst end-to-end when accessed via free-text (as opposed to the `/playbook` slash-command, which DOES skip agent-loop overhead entirely and would show the expected speedup).

(Both the "Tong hop Finding theo don vi" residual-issue and SYSTEM_PROMPT-trim-insufficient observations from live Bot 2 verification are folded into the consolidated "Aggregate-query token budget" entry above.)

- [FIXED] **Post-QRadar-removal live retest**: generate_sigma_rule's "include full YAML verbatim" instruction lost when the turn hit _force_final_answer()
  User ran "Cho toi CVE nghiem trong nhat cua EVNNPC tuan nay va Sigma rule tuong ung" live on Bot 2 (unrelated to the QRadar/AQL removal earlier in this session -- confirmed by reproducing directly). Result: agent found the correct (if low-confidence, community_behavioral) Sigma rule via `generate_sigma_rule`, but PARAPHRASED it into prose (title, logsource, detection keywords as bullet points) instead of pasting the raw YAML verbatim, despite the tool's own description explicitly instructing "include the FULL raw YAML ... verbatim ... do not paraphrase" (added earlier this session, still present and unchanged in `agent/tools/generate_sigma_rule.py`).
  Root cause: the turn used 66,936 tokens, over `TOKEN_SOFT_CAP=50_000`, so `function_calling.py` routed to `_force_final_answer()` -- a SEPARATE LLM call whose own user-message instruction says "Tra loi ngan gon" (answer concisely), with no mention of the YAML-verbatim requirement from the tool description. The two instructions directly conflicted (verbose YAML vs. "be concise"), and the model resolved it by prioritizing the more recent/specific-sounding instruction (conciseness) over the tool description's requirement.
  Fixed: `_force_final_answer()`'s forced-summary message (function_calling.py) now carries an explicit exception -- stay concise EXCEPT when a prior tool result contains content that must be preserved verbatim per that tool's own instructions (Sigma YAML from generate_sigma_rule/search_sigma_rules, playbook markdown from generate_playbook/get_playbook) -- in that case paste it in full, only trim the surrounding commentary. Verified by reproducing the exact failing question via `test_agent.py`: still hits the token cap and `_force_final_answer()` (74,822 tokens this run), but now correctly pastes the full Sigma YAML in a code block instead of paraphrasing it.

- [FIXED] **Live retest**: raw DeepSeek tool-call special tokens leaked into the analyst-facing answer + "CVE nao co nhieu Finding nhat" paginated the whole dataset again
  User ran "Sinh Sigma rule cho CVE co nhieu Finding nhat" live on Bot 2. Two bugs in one turn: (1) the FINAL message sent to the analyst was literal, unparsed DeepSeek special-token markup ("<｜DSML｜tool calls>\n<｜DSML｜invoke name=\"generate sigma rule\">..."), because the model attempted a 7th tool call by writing it as raw text instead of the API's structured `tool_calls` field -- `function_calling.py`'s "no tool_calls -> treat as final answer" branch had no guard against this and sent it straight to Telegram; (2) before that, the model tried to answer "CVE nao co nhieu Finding nhat" the same expensive way as the earlier-fixed "tong hop theo don vi" case -- 6 `search_findings` calls with increasing `offset` to read all ~200 Findings and count manually (92,420 tokens, near-total failure), because no aggregate tool existed for "most Findings per CVE" specifically (only per-customer, via `generate_report`, and per-ATT&CK-technique, via `top_attack_techniques`).
  Fixed: (a) added `_RAW_TOOL_SYNTAX_RE` (function_calling.py) matching the DeepSeek DSML markup pattern (`<｜DSML｜tool calls>` / `<｜DSML｜invoke ...>`, both full-width "｜" and a plain "|" fallback) -- when content matches, raise `FunctionCallingFailure` instead of returning it as a valid answer, which runner.py already handles by retrying function-calling once then falling back to ReAct (plain-text Thought/Action format, doesn't depend on the provider parsing structured tool_calls at all, so immune to this specific leak); (b) added a new aggregate tool `top_cve_by_finding_count` (agent/tools/top_cve_by_finding_count.py, same GROUP BY pattern as the earlier `top_attack_techniques` fix) plus a `SYSTEM_PROMPT` tool-selection rule pointing "CVE nao co nhieu Finding nhat" at it instead of manual pagination.
  Verified via `test_agent.py` reproducing the exact question: now 2 tool calls (`top_cve_by_finding_count` -> `generate_sigma_rule`) instead of 6+1, 64,510 tokens (vs 92,420), correct CVE identified (cve-2025-68686, verified independently via direct SQL: 3 Findings, the actual max), full AI-generated Sigma YAML pasted verbatim, no raw-syntax leak.

- [FIXED] **Live retest**: REACT_SYSTEM missing the entire "Tool selection heuristics" section that SYSTEM_PROMPT has
  User retested "Sinh Sigma rule cho CVE co nhieu Finding nhat" after the prior fix -- this time function-calling itself timed out (unrelated to the fixes above, just a slow provider round), correctly falling back to ReAct. But the ReAct answer called `generate_sigma_rule(cve_id=cve-2025-68686)` WITHOUT `force_regen=True`, defaulting to the "tim/tra" (look-up) behavior and returning an unrelated low-relevance community rule ("Cisco Discovery") instead of the AI-generated rule the analyst asked for by saying "Sinh" (an explicit generate request). Root cause: `REACT_SYSTEM` (agent/loop/react.py) never had ANY of `SYSTEM_PROMPT`'s "Tool selection heuristics" section -- only a short generic `Rules:` block -- so every domain-specific mapping in that section (force_regen sinh/tim distinction, generate_report for per-customer breakdowns, top_cve_by_finding_count for most-Findings-CVE, top_attack_techniques, campaign status defaults, playbook tool choice, etc.) was silently unavailable whenever function-calling failed and ReAct took over -- a structural gap that would keep resurfacing for every future tool-selection rule unless fixed at the source.
  Fixed: extracted the entire "Tool selection heuristics" block into a new shared `TOOL_SELECTION_HEURISTICS` constant (agent/loop/config.py, defined once, substituted into both `SYSTEM_PROMPT` and `REACT_SYSTEM` via the same `{placeholder}.replace()` pattern already used for `EVN_SCOPE_RULES`) so any future addition only needs to happen in one place and both loop modes see it automatically. (Encountered and cleaned up a self-inflicted duplicate-content mess mid-edit from an earlier attempt at this same extraction -- verified via `ast.parse` after cleanup that the file is syntactically clean with no leftover duplicated blocks.)
  Verified via `test_agent.py --force-react` reproducing the exact question: ReAct now correctly calls `generate_sigma_rule(cve_id=cve-2025-68686, force_regen=True)`, producing the AI-generated Sigma YAML specific to the CVE instead of the unrelated community rule.

- [OBSERVED, low priority] **Live retest**: function-calling repeatedly times out (60s) on Sigma-rule-generation questions, but ReAct fallback now gives the correct answer thanks to the 3 fixes above
  Confirmed all 3 fixes above (top_cve_by_finding_count, raw-syntax guard, shared TOOL_SELECTION_HEURISTICS) work correctly via a second live Bot 2 retest of "Sinh Sigma rule cho CVE co nhieu Finding nhat": correct CVE identified (cve-2025-68686, 3 Findings), correct AI-generated (force_regen=True) Sigma YAML pasted verbatim, no raw-syntax leak. However `⚠ Fallback: Timeout after 60s` still appears every time this question type is tested -- likely because `generate_sigma_rule(force_regen=True)` internally makes its own LLM call (`sigma_generator.py`, `max_tokens=4096`, no explicit per-call timeout override) to AI-generate the YAML, ON TOP OF the function-calling loop's own LLM calls to decide which tools to invoke -- the combined latency for a multi-step turn (aggregate lookup + AI generation) can exceed `TIMEOUT_SECONDS=60`, which wraps the ENTIRE function-calling turn (agent/loop/runner.py), not a single step.
  Not fixed this session -- deliberately deferred: `TIMEOUT_SECONDS` is a global constant affecting every question type, not just Sigma-rule generation, so raising it needs broader consideration of the UX tradeoff (analyst waits longer on ALL slow turns, not just this one) rather than a reactive bump. Since the ReAct fallback path now produces a CORRECT answer (unlike before this session's fixes), this is a performance/UX nit (analyst waits ~76s total: 60s failed function-calling attempt + ~16s successful ReAct) rather than a functional bug. Worth revisiting if AI-generated Sigma rule / playbook requests turn out to hit this timeout frequently enough in practice to annoy analysts -- possible future fix: a per-tool timeout hint that function_calling.py could use to decide whether to keep waiting past the global TIMEOUT_SECONDS when the most recent tool call is a known-slow one (LLM-backed generation), rather than changing the global constant.

- [FIXED] **Live retest**: generate_sigma_rule's YAML pasted without the required ```yaml code fence, losing Telegram monospace rendering
  Screenshot from live Bot 2 confirmed a Sigma rule from "sinh sigma rule cho no" rendered as plain unformatted text -- no monospace, blank list items visually broken -- because the model pasted the YAML content verbatim (correctly, per the "do not paraphrase" instruction fixed earlier) but WITHOUT the ```yaml code fence the tool description also explicitly requests. Same behavioral-discipline class of gap as prior fixes -- the instruction exists but isn't reliably followed.
  Fixed (agent/loop/postfilter.py) at the code layer instead of trying yet another prompt wording: `_ensure_yaml_fenced()` detects an unfenced Sigma-rule-shaped block (a bare `title:` line followed later by `logsource:` and `detection:` lines with no ``` anywhere in the answer) and wraps it in a ```yaml fence, called at the start of `sanitize_telegram_markdown()`. While building this, also caught and fixed a related pre-existing bug it would have made visible more often: `_SNAKE_CASE_RE` and `_NESTED_DASH_RE` (the underscore-to-space and dash-to-plus rewriters, both meant for PROSE) were rewriting content INSIDE ```-fenced code blocks too, corrupting real YAML syntax (e.g. `wsrep_notify_cmd` -> `wsrep notify cmd`, YAML list `-` markers -> `+`, both invalid YAML) whenever a code fence happened to be present. Fixed by splitting the answer on fenced segments and only applying prose-rewriting regexes to the segments OUTSIDE fences, reassembling unchanged.
  Verified via `sanitize_telegram_markdown()` directly: an unfenced Sigma YAML block gets wrapped in ```yaml with all field names and list dashes intact; an already-correctly-fenced YAML block is left untouched; prose text outside any fence (e.g. a stray "risk_score" mention) still gets its snake_case rewritten as before.
